/* Insert into dbo.item using OPENROWSET(BULK ''' + @FullName + ''', SINGLE_BLOB) 
   into the VARBINARY MAX column.  Using a cursor to grab all of the files */
...
/* Check that there is 192 records 
    No need to display the binary data in ItemImage column */
...
SELECT *
FROM sys.dm_fts_index_keywords(DB_ID('WhatsUpDoc_Database'), OBJECT_ID('item'))
